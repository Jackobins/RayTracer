...
int main() {
    ...
    right->setTransform(matrixOps::multiply(matrixOps::translationMatrix(1.5,0.5,-0.5),
                                        matrixOps::scalingMatrix(0.7,0.7,0.7)));
    right->surfaceMaterial = material();
    right->surfaceMaterial.diffuse = 0.2;
    right->surfaceMaterial.specular = 1;
    right->surfaceMaterial.shininess = 300;
    right->surfaceMaterial.transparency = 0.9;
    right->surfaceMaterial.reflective = 0.9;
    right->surfaceMaterial.refractiveIndex = 1.52;
    ...
    camera* cam = new camera(852, 480, M_PI/3);
